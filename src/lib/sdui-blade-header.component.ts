...
@Component({
  ...
  host: { class: 'block' },
  template: `
    <div class="sdui-blade-header sticky top-0 bg-[var(--th-panel-bg,#1e293b)]/90 backdrop-blur z-10 border-b border-[var(--th-border,#334155)] min-h-[64px] px-[var(--sdui-blade-px,20px)] shrink-0 flex items-center justify-between">
      <div class="sdui-blade-title flex items-center gap-3">
        ...
        <div class="sdui-blade-title-text-group flex flex-col">
          <!-- The Title Row -->
          <div class="sdui-blade-title-row flex items-center gap-2 empty:hidden">
          ...
          <!-- The Subtitle Row -->
          <div class="sdui-blade-subtitle-row flex items-center gap-1.5 mt-0.5 empty:hidden">
            <ng-content select="[subtitle-left]"></ng-content>
            @if (subtitle()) {
              <p class="sdui-blade-subtitle-text text-xs text-[var(--th-text-secondary,#94a3b8)]">{{ subtitle() }}</p>
            }
