...
@Component({
  ...
  template: `
    <div class="sdui-mock-blade-container flex flex-col flex-1 h-full min-h-0 w-full text-[var(--sdui-text)]">
      <sdui-blade-header 
      ...
      @if (($any(node().properties['width']) || 'full') !== 'full') {
        <sdui-blade-footer>
          ...
          <button class="sdui-mock-save-btn bg-[var(--sdui-border)]/30 text-[var(--sdui-muted)] px-[18px] py-[6px] rounded-[2px] text-[13px] font-semibold flex items-center justify-center cursor-not-allowed border border-[var(--sdui-border)]">Save</button>
          <!-- Azure Active Secondary Button Mock (Cancel) -->
          <button class="sdui-mock-discard-btn bg-transparent text-[var(--sdui-primary)] px-[18px] py-[6px] rounded-[2px] text-[13px] font-semibold transition-colors hover:bg-[var(--sdui-primary)]/10 border border-[var(--sdui-primary)] flex items-center justify-center cursor-pointer" (click)="bladeService.closeBlade(node().id)">Discard</button>
        </sdui-blade-footer>
      }
    </div>
  `
